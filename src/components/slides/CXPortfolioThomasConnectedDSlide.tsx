...
const PIECE_FILLS = [
  ...
  "rgba(255,248,242,0.10)",
  "rgba(250,252,255,0.095)",
];
...
export default function CXPortfolioThomasConnectedDSlide({ slideNumber = 59 }: { slideNumber?: number }) {
  return (
    <div
      className="slide"
      style={{
      ...
      data-speaker-notes="Four products. One integrated platform — visualized as interlocking puzzle pieces."
    >
      <motion.header
      ...
      >
        <p style={{ fontWeight: 700, fontSize: 15, letterSpacing: "0.08em", textTransform: "uppercase" as const, color: ACCENT, margin: "0 0 10px" }}>Our solution</p>
      ...
      >
        ...
        >
          <defs>
            <filter id="puzzle-shadow-d" x="-20%" y="-20%" width="140%" height="140%">
              <feDropShadow dx="0" dy="2" stdDeviation="4" floodColor="rgba(0,0,0,0.35)" />
            </filter>
            {PIECES.map((p, i) => (
              <clipPath key={p.name} id={`clip-d-${i}`}>
                <path d={p.path} />
              </clipPath>
            ))}
          </defs>
          {PIECES.map((piece, i) => (
            <g key={piece.name} clipPath={`url(#clip-d-${i})`}>
              <motion.path
                d={piece.path}
                fill={PIECE_FILLS[i]}
                stroke={`rgba(40,96,178,0.35)`}
                strokeWidth={2.5}
                strokeLinejoin="round"
                filter="url(#puzzle-shadow-d)"
                ...
                >
                  <div style={{ marginBottom: piece.y === 0 ? 2 : 3, display: "flex", flexDirection: "column", alignItems: "center" }}>
                    ...
                    <span style={{
                      ...
                      background: "rgba(40,96,178,0.2)", color: ACCENT,
                  ...
                  <p style={{
                    ...
                    fontFamily: "'Space Grotesk', sans-serif",
                    paddingLeft: (piece.name === "XBert" || piece.name === "NEXT") ? RIGHT_COL_PADDING_LEFT : 0,
                  }}>
                    {piece.tagline}
                  </p>
                  <ul style={{
                    margin: 0, paddingLeft: (piece.name === "XBert" || piece.name === "NEXT") ? RIGHT_COL_PADDING_LEFT : 0, listStyle: "none", flex: 1,
